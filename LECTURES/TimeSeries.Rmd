...
* When is our data suited to using an interrupted time series model? 
* How should you organize your data before using an interrupted time series?
* How do you interpret the coefficients of an interrupted time series? 
* What are the two counterfactuals in the model? 
* What does it mean for the second counterfactual to vary with time? 
* What are the main threats to validity when running a time series?
* What is autocorrelation? How do we test for it? 
* How do we implement a time series model in R? 
...
For our first example, let's assume we have data on the total consumption of tobacco in a US state from a point in time (t0) to ten years later (t10). Our observations are represented in the graph \@ref(fig:f1) below. The line suggests an upward trend, with state residents consuming more tobacco over time. 
