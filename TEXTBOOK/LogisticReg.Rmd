...
install.packages( "sandwich", repos="https://cran.cnr.berkeley.edu" )
install.packages( "lmtest", repos="https://cran.cnr.berkeley.edu" )
install.packages( "margins", repos="https://cran.cnr.berkeley.edu" )
...
We start with a simple example to illustrate the **Linear Probability Model**. We want to estimate the probability that a student will be admitted to law school based on [some key variables](https://www.usnews.com/education/blogs/law-admissions-lowdown/2012/11/12/learn-the-5-deciding-factors-in-law-school-admissions). Admission to law school can be challenging, although there are claims that the [average score for admission has decreased](https://www.nytimes.com/2015/10/27/business/dealbook/study-cites-lower-standards-in-law-school-admissions.html) in the past years and there are [disparities in admission and job market for female and male students](https://www.nytimes.com/2016/11/30/business/dealbook/more-law-degrees-for-women-but-fewer-good-jobs.html)
...
As before, we use the *lm* function to estimate our coefficients. Reslts are summarized in the table below.
...
Results from a LPM can be interpreted in the same way we interpret results from an OLS model. However, in a LPM we refer to a probability outcome. For instance, for each one-unit increase in the LSAT score, the probability of the event to occur increases of 0.019. For a one unit increase in the Essay score, the probability increases of 0.010. Finally, for one unit increase in the GPA, the probability of being admitted increases of 0.132. 

## Limitations of the LPM

Yet there are some limitations when using a LPM with a binary variable. 

1. First, the model suffers from *heteroskedasticity*. Heteroskedasticity occurs when the variance of the error terms is not constant but depends on X1. We can see this graphically by plotting the residuals against the predict values. If the model was homoskedasticc, we would expect the residuals to be randomly distributed.   

```{r residuals, fig.cap= "Residuslas vs fitted values"}
...
You might remember that we can use *robust standard errors* to overcome heteroskedasticity. Robust standard errors allow the the variance to be difference across observations.
...
In this case, the standard errors have little change - for GPA, the standard error decreases from 0.016 to 0.0149.The essay standard error decreases from 0.001 to 0.0006. 

2. Second, a LPM assumes a linear relationships between Y and the independent variables. A linear relationship might be a good approximation when the variable is continuous but it is not necessarily a good approximation when the variable is binary.

In OLS models, we assume a linear relationship between X and Y. When using an OLS with binary data, it's like we are calculating the average probabilities across groups of observations based on their independent variables. 

For instance, let's order our observations by LSAT score and take the 50 students with the lowest LSAT score. The average LSAT score is  131.44. The average probability is equal to 0.16. We can plot this point on the our graph. 

Then we can take the average probability for the 51st observation to the 100th observations. The probability is equal to 0.34 - we expect an higher probability as the average LSAT score has also increased to 137.54. 
...
points( mean( df$LSAT[ 1:50 ] ), mean( df$Admission[ 1:50 ] ), col = "red", lwd = 4)
points( mean( df$LSAT[ 51:100 ] ), mean( df$Admission[ 51:100 ] ), col = "red", lwd = 4)

```

If we do this process for all observations and we plot the results on the graph, we would have something like this. 

We also add the regression line for the a linear probability model including only the LSAT score. 

```{r}

df <- df[order(df$LSAT),] 

df$d = c(rep(1,50), rep(2,50), rep(3,50), rep(4,50), rep(5,50), rep(6,50),
rep(7,50), rep(8,50), rep(9,50), rep(10,50), rep(11,50), rep(12,50), rep(13,50), rep(14,50), rep(15,50), rep(16,50),
...
plot(df$LSAT, df$Admission, col = adjustcolor("gray", alpha.f= 0.3), pch=19, bty="n", cex = 2, xlab="LSAT", ylab="Admission into law school")
points(gr_mean2$x, gr_mean$x, col = "red", lwd = 4)
abline(lm(df$Admission ~ df$LSAT), col = "darkred", lwd = 3)
...
* Because of this last point, several persons prefer a logit model over a LPM. In the logit, we move from a line to a s-shaped curve (see \@ref(the-logit-model))

3. Third, an OLS model does not constraint the predicted values to be between 0 and 1 as a probability model would require. As a result, we can have negative predicted values or predicted values that are greater than 1. For instance, you can look at the descriptive statistics of yhat of the previous model. Values range from -0.015 to 1.30. But a probability greater than 1 is not realistic. This is another reason why most people prefer a logit model. A logit model will contraint the outcome variable to range from 0 to 1. 
...
We utilize a logistic (or logit) model, which relies on S-shaped logistic function to estimate the likelihood of the event to occur. 

We can see in the same example we discussed before. We plot again the LSAT score against the probability of being admitted to law school. We also add the average probability per group of observations. 

But instead than plotting a straight line as in the LPM, we plot the s-shape function of the logit model. While there are not big differences in the middle of the graph, the s-shaped function better approximates the values at the two extremes. 
...
In order to utilize a s-shaped function, we need to move from an OLS model to a Maximum Likelihood estimator. It is beyond the scope of this lecture to enter into detailes about maximum likelihood estimation. So, we present here only some key points: 

1. The Maximum Likelihood (ML) is an estimation method (like OLS). 

An estimation method is a statistical technique that helps us to find a set of parameters (the coefficients) that generate the data. 

2. The ML estimator requires assumptions about the distribution function - in other words, it wants to know the basic process that generated the data. 
...
3. Once we decide a distribution function, ML tries to find the most likely values (our coefficients) that would explain the data (outcomes) we observe according to our distribution function.

The ML estimator utilizes an iterative process - in simple words, it tries several combination of values until it finds the one that gets closer to the real data. The output is a set of a coefficient that allows us to predict our dependent variable. 

Note that When using a statistical software, the process is based on various algorithms trying different combinations of coefficient.  

4. At the end of the process, the ML estimator returns our coefficients, standard errors, and t-test as an OLS model. Moreover, we get a likelihood function which measures the probability of observing our dependent variable values given our chosen set of coefficients. As you might imagine, the scope of MLE is to maximize the likelihood function. 

### An example

The Maximum Likelihood is easy to implement. The *glm* function in R utilizes a Maximum Likelihood Estimator to calculate the coefficients. We can use the same data we used for the LPM (see \@ref(linear-probability-model-(pm))) to run a logit example to predict the probabilities of being admitted to law school. 
...
As you can see the output does not fundamentally change from an OLS model. We have the coefficient, standard errors and significance levels. 

The two parameter reported at the bottom of the table are the Log Likelihood and the Akaike Inferior Criteria (AIC). Both values are per se not meaningful but we will use them to compare across models.  

* the log likelihood is the logarithm of the likelihood function - i.e., the probability of observing our actual dependent variables based on the coefficient we obtain (\@ref(maximum-likelihood-estimation)). We can use the log likelihood to compare across models. The lower the log-likelihood, the more the model is a better fit to the data. 

* the AIC represents how much information gets lost when estimating the model. Each model looses some information (in fact, each model has residuals!). Higher quality model will have a lower AIC indicating the less information has been lost. 

While there is not a "good" AIC or a "good" log likelihood, you can use these two parameter to compare models. 

## Interpretation

The tricky part of logit model is to interpret coefficients. In this section we explain why it is problematic, before discussing an example. 

One problem with modeling probability is the limited range of the variable, from 0 to 1. We discussed it before in the LPM; one limitation of the LPM is not taking into account the restricted range of values for the dependent variable (\@ref(limitations-of-the-lpm)). To solve this problem, the dependent variable is transformed from a binary variable to log odds.
...
We naturally transform the dependent variable into **probabilities** when we consider its distribution to be a function of s-shaped curve moving from 0 to 1. 

**Odds ratios** are a probability ratio. If the probability of an event to occur is equal to 0.4, then the probability of not occurring is 0.6 (= 1 - 0.4). The odds are: 

<center>$\text{Prob of Success  / Prob of Failure}$</center>
<br>
in other words 0.4 / 0.6 = 0.67, so as the probability of an event to occur is 0.67 against 1. 

We can make another example. If an event has a probability to occur equal to 0.8, then its probability not to occur is 0.2. The odds ratio is 0.8/0.2 = 4. The event is 4 times more likely to occur than not to occur.

We can look at the relationship between  probabilities and odds ratio in graph \@ref(fig:prOR). They are positive correlated as an increase in the probability increases the odds ratio. 

Also, note that odd ratios can assume values greater than 1. Yet odd ratios can assume only positive values, whereas an odds ratio grater than 1 means that success is more likely to occur, while an odds ratio lower than one indicate that failure is more likely to occur. 
...
To further transform our binary variable into a continuous one, we transform the odds ratio into **log odds** using a logarithm transformation. This is called the *logit transformation*. 

Again, we can plot the odds ratios against the log odds. As you can see, log odds can assume both poistive and negative values.
...
Log odds represent the unit of analysis of our coefficients. Each coefficient represent a one-unit change in the log odds that the event will occur. 

While the transformation from binary to log odds facilitate the model estimation, it does not help with interpretation. 

1. The log odds that are not very meaningful. So, we need to tranform back our coefficients into odds ratio or probabilities, which are easier to interpret. 
2. Yet probabilities are distributes along a non-straight line. While in linear models, the relationship between X and Y is constant (e.g., it is represented by the slope of the line as in figure \@ref(fig:linear)), in non-linear models the relationship is not constant but depend on the value of x (look again at graph \@ref(fig:binaryfunction)).
