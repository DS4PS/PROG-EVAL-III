...
```{r, echo=FALSE, results="hide", cache=F}
...
knitr::include_graphics( "O:/Users/ffusi/Jesse/IV/Picture2.png" )
```
...
When we omit a variable that is correlated with the policy and the outcome variable, it affects both the slope and the standard error of the model - you can observe this in figure \@ref(fig:figure2). Including or excluding X2 significantly changes the covariance between Y and X1 (the intersection between Y and X1) and the residual of Y (the part of Y which does not intersect with X1 or X2).   

```{r figure2, fig.cap="Covariance structure and residuals", out.width='80%', echo=F }
knitr::include_graphics( "O:/Users/ffusi/Jesse/IV/Cov_Structure.png" )
```
...
knitr::include_graphics( "O:/Users/ffusi/Jesse/IV/Picture3.png" )
```
...
knitr::include_graphics( "O:/Users/ffusi/Jesse/IV/Picture6.png" )
```
...
knitr::include_graphics( "O:/Users/ffusi/Jesse/IV/Picture7.png" )
```
...
knitr::include_graphics( "O:/Users/ffusi/Jesse/Diff in Diff/Graduation.jpg" )
```
...
knitr::include_graphics( "O:/Users/ffusi/Jesse/IV/Ex_Model.png" )
```
...
<center>$Y=\beta_0 + \beta_1\text{Education} + \beta_2\text{Ability} + \beta_3\text{Control}$</center>


Where:

$\beta_3$ =  Control variable (gender, geographical location, experience...)
...
<center>$Y=\text{b}_0 + \text{b}_1\text{Education} + \text{b}_3\text{Control}$</center>
<br>
<br>
We can now compare the different estimates: 

```{r, echo=T, results='asis'}
full.model  <- lm( y ~ x1 + x2 + x3 ) # Full Model if we were able to measure ability
naive.model <- lm( y ~ x1 + x3 )      # Naive Model where ability is excluded (omitted variable bias)

stargazer( full.model, naive.model, 
           type = "html", 
           dep.var.labels = ("Wage"),
           column.labels = c("Full Model", "Naive Model"),
           covariate.labels = c("Education", "Ability", "Control"),
           omit.stat = "all", 
           digits = 2 )
```
...
plot(z, x1, xlab = "Father's education", ylab = "Education" )   # Plot

abline( lm( x1 ~ z ), col = "red")                              # regression line 

plot( z, y, xlab = "Father's education", ylab = "Hourly wage" )

abline(lm( y ~ z ), col = "green") 

plot( z, x2, xlab = "Father's education", ylab = "Ability")

abline(lm( x2 ~ z ), col = "blue")
```
...
<center>$Education=\beta_0 + \beta_1\text{Father's education} + \beta_2\text{Control variable}$</center>

```{r, results='asis'}
first.stage  <- lm( x1 ~ z + x3)        #Run the first stage predict the education variable
...
stargazer( first.stage,
           ...
           covariate.labels = c("Father's education", "Control"),
           digits = 2 )
```
...
<center>$Wage=\beta_0 + \beta_1\text{Education} + \beta_2\text{Control variable}$</center>

```{r, results='asis'}
x1_hat       <- fitted( first.stage )   
#Saved the predicted values of education from the first stage

second.stage <- lm( y ~ x1_hat + x3 )   
...
stargazer( full.model, naive.model, second.stage,
           ...
           omit.stat = c("adj.rsq", "ser"), 
           covariate.labels = c("Education", "Ability", "Predicted Education", "Control"),
           digits = 2 )
```
...
plot(x2, x1, xlab = "Ability", ylab = "Education")

abline(lm( x1 ~ x2 ), col = "green")
```
...
knitr::include_graphics( "O:/Users/ffusi/Jesse/IV/Ex_Partition.png" )
```
...
first.stage <- lm( x1 ~ z + x3 )          #Run again the first stage to predict education (x1)
...
plot( x2, x1_hat, xlab = "Ability (omitted variable)", ylab = "Predicted values of Education (x1_hat)")

abline( lm( x1_hat ~ x2), col = "blue")

#Plot the residual against ability
plot( x2, e_x1, xlab = "Ability (omitted variable)", ylab = "Education - Residual (e_X1)" )

abline( lm( e_x1 ~ x2), col = "green")
```
...
plot( x1, y, xlab = "Education", ylab = "Wage" )

abline( lm( y ~ x1), col = "red")

plot( x1_hat, y, xlab = "Predicted values of Education (x1_hat)", ylab = "Wage" )

abline( lm( y ~ x1_hat), col = "blue")
```
...
knitr::include_graphics( "O:/Users/ffusi/Jesse/IV/PathDiagram_2.png" )
```
...
* X2 is the omitted variable
* X3 is a control variable
